...
def render(root: Path) -> str:
    ...
    lines.append("- `start.ps1 -Brain -Dashboard` (recommended default)")
    lines.append("- `start.ps1 -Mouth` (single-terminal REPL -> NeuralBus -> Executive Core)")
    lines.append("- `scripts/sovereign_brain.py` (direct brain entry)")
    lines.append("")
    lines.append("## Intelligence Routing")
    lines.append("- Cloud vs local planning: `scripts/executive_core.py`")
    lines.append("- Local fallback model (default): `phi4` via Ollama")
    lines.append("- Resource balancer: `scripts/resource_balancer.py` + `agents/skills/skill_resource_balancer.py`")
    lines.append("")
    lines.append("## Governance")
    lines.append("- Auto-governor audit: `scripts/auto_governor.py` -> `state/decision.json`")
    lines.append("- Interactive gate (manual override): `scripts/governance_gate.py`")
    lines.append("")
    ...
    lines.append("- Zero-gap audit trail generator: `scripts/audit_trail.py`")
    lines.append("- Test harness runner: `scripts/redteam_suite.ps1`")
    lines.append("")
    lines.append("## Skills")
    ...
    lines.append("")
    lines.append("## Config")
    lines.append("- Shared profiles: `configs/config.base.toml`, `configs/config.core.toml`, `configs/config.full.toml`, `configs/config.max.toml`")
    lines.append("- Local overrides (ignored): `configs/config.local.toml`")
    lines.append("- Runtime assembled (ignored): `configs/config.active.toml`")
    lines.append("")
