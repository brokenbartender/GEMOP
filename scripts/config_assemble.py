...
def assemble(base_path: Path, profile_path: Path, local_path: Path) -> str:
    ...
    if base:
        parts.append("# --- configs/config.base.toml ---")
        parts.append(base)
...
def main() -> int:
    ...
    ap.add_argument("--out", default="", help="Output path (default: configs/config.active.toml)")
    args = ap.parse_args()
    ...
    profile_path = configs_dir / f"config.{args.config_profile}.toml"
